...
class CRNN_lyrics_pitch_onset(nn.Module):
    ...
    def __init__(
        ...
        **args
        ...
        # self.pooling_pitches_onsets = nn.MaxPool1d(
        #     kernel_size=3,
        #     stride=self.down_sample, 
        #     padding=get_padding(3),
        # )
        ...
        self.rnn_lyrics = nn.Sequential(
            *[
            RNNLayer(
            input_size=lstm_channels * 2 if i > 0 else conv_channels[-1] * downsample_length(input_features, self.down_sample) + 129 + 1, 
    ...
    def forward(self, x, pitches, onsets):
        ...
        # pitches: (batch_size, 129, length)
        pitches_onsets = torch.cat([pitches, onsets], dim=1)

        x = self.cnn(x)
        # x: (batch_size, channel=conv_channels, new_feature_num, new_length)
          
        x_lyrics = self.pooling_lyrics(x)
        pitches_onsets = torch.nn.functional.interpolate(pitches_onsets, size=x_lyrics.shape[-1])
        ...
        x_lyrics = torch.cat([x_lyrics, pitches_onsets], dim=1)
